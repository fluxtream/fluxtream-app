...
%><%@ page import="net.tanesha.recaptcha.*" %><%
	...
	if (request.getAttribute("errors")!=null) errors = (ArrayList<String>) request.getAttribute("errors");
	
	List<String> required = new ArrayList<String>();
	if (request.getAttribute("required")!=null) required = (ArrayList<String>) request.getAttribute("required");
	
	String invitationCode = "";
	if (request.getAttribute("invitationCode")!=null) invitationCode = (String) request.getAttribute("invitationCode");
...
%><div id="createAccountFormContainer" class="modalForm">
	<h2>Create your account</h2>
	<table class="modalFormNewAccount">
		<tr>
			<td valign="top"><label>E-mail</label></td>
			<td><input id="email" name="email" value="<%=email%>" type="text"></input>
			...
			<td><input id="username" name="username" value="<%=username%>"></input>
				...
				<% } else if (errors.contains("usernameTaken")) { %>
					<div class="formErrorCont">
						<div class="icon"></div>
						<span class="formError username">This username is already taken</span>
					</div>
				<% } %>
			</td>
		</tr>
		<tr>
			...
			<td><input id="firstname" name="firstname" value="<%=firstname%>"></input></td>
		</tr>
		...
		<tr><td valign="top"><label>Password</label></td>
		<td><input type="password" name="password1" id="password1"></input><br>
			...
			<% } else if (errors.contains("passwordTooShort")) { %>
					<div class="formErrorCont">
						<div class="icon"></div>
						<span class="formError">Password must be at least 8 characters long</span>
					</div>
		...
		<tr><td valign="top"><label>Re-type password</label></td>
		<td valign="top"><input type="password" name="password2" id="password2"></input><br>
		...
		<tr><td colspan="3" id="recaptcha"></td></tr>
		<% if (errors.contains("wrongCaptcha")) { %>
		<tr><td colspan="3">Wrong Captcha answer. Are you human?</td></tr>
		<% } %>
		<td colspan="3" align="right"><button onclick="submitCreateAccountForm()" class="btn primary">Create Account</button></td></tr>
	</table>
</div>