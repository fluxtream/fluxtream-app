...
public class PartnerAppsStore {
    ...
    public StatusModel createApplication(@FormParam("name") String name,
                                         @FormParam("description") String description) {
        final long guestId = AuthHelper.getGuestId();
        try {
            partnerAppsService.createApplication(guestId, name, description);
            StatusModel status = new StatusModel(true, String.format("Created app '%s': %s", name, description));
            return status;
        }
        catch (Throwable e) {
            final String stackTrace = ExceptionUtils.getStackTrace(e);
            final StatusModel statusModel = new StatusModel(false, String.format("Could not create app '%s'", name));
