 ...
 * Time: 21:32
 */
public class FlxDeveloperWebsiteAuthFilter extends UsernamePasswordAuthenticationFilter {
    ...
    public Authentication attemptAuthentication(
        ...
        final FlxUserDetails principal = (FlxUserDetails) authentication.getPrincipal();
        if (authentication.isAuthenticated()&&
            !principal.getGuest().getUserRoles().contains("ROLE_DEVELOPER")){
            guestService.addDeveloperRole(principal.getGuest().getId());
            throw new DeveloperRoleAddedException("Hi " + principal.getUsername() + "! We just registered you as a developer. " +
                    "Please log in again for this change to take effect. This will happen only once");
        }
