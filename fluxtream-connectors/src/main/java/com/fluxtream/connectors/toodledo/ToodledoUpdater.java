...
public class ToodledoUpdater extends AbstractUpdater {
	...
	private void sync(UpdateInfo updateInfo) throws Exception {
		...
		String mostRecentJsonString = restHelper.makeRestCall(connector(),
				updateInfo.getGuestId(), -1, url);
		...
		String lastAccountInfoJsonString = guestService.getApiKeyAttribute(
				updateInfo.getGuestId(), connector(), "lastAccountInfo");
	...
	private void retrieveGoals(UpdateInfo updateInfo, long since, boolean update)
		...
		String goalsJson = restHelper.makeRestCall(connector(),
				updateInfo.getGuestId(), 2, urlString);
	...
	private void retrieveTasks(UpdateInfo updateInfo, long since, boolean update)
		...
		String urlString = TOODLEDO_TASKS_GET + "?key=" + key + "&after="
				+ since;
		String tasksJson = restHelper.makeRestCall(connector(),
				updateInfo.getGuestId(), 1, urlString);
		...
		if (update) {
			urlString = TOODLEDO_TASKS_GET_DELETED + "?key=" + key + "&after="
					+ since;
			String tasksToDeleteJson = restHelper.makeRestCall(connector(),
					updateInfo.getGuestId(), 1, urlString);
	...
	private String getKey(UpdateInfo updateInfo) throws Exception {
		...
		String userPassword = guestService.getApiKeyAttribute(
				updateInfo.getGuestId(), connector(), "password");
	...
	private String getSessionToken(UpdateInfo updateInfo) throws Exception {
		String token = "";
		String sessionTokenExpires = guestService.getApiKeyAttribute(
				updateInfo.getGuestId(), connector(), "sessionToken-expires");
		long now = System.currentTimeMillis();
		if (sessionTokenExpires == null
			...
			String userid = guestService.getApiKeyAttribute(
					updateInfo.getGuestId(), connector(), "userid");
			token = getSessionToken(updateInfo, userid);
			guestService.setApiKeyAttribute(updateInfo.getGuestId(),
					connector(), "sessionToken", token);
			String fourHoursFromNow = String.valueOf(now + (3600000 * 4));
			guestService.setApiKeyAttribute(updateInfo.getGuestId(),
					connector(), "sessionToken-expires",
					fourHoursFromNow);
		} else
			token = guestService.getApiKeyAttribute(updateInfo.getGuestId(),
					connector(), "sessionToken");
		return token;
	}
	...
	public String getSessionToken(UpdateInfo updateInfo, String userid)
		...
		String url = TOODLEDO_ACCOUNT_TOKEN + "?userid=" + userid + ";appid="
				+ appId + ";sig=" + signature;
		try {
			String json = restHelper.makeRestCall(connector(),
					updateInfo.getGuestId(), -1, url);
	...
	public String getToodledoUserid(long guestId, String email, String password)
		...
		String json = restHelper.makeRestCall(connector(), guestId, -1, url);
