 ...
 *  }
 *
 * Chris Bartley <bartley@cmu.edu> made these modifications on 2012.02.16:
 * - fixed formatting
 * - fixed minor syntax issues
 * - added input validation, based on example code by David Renty at https://github.com/webworka/Tagedit/issues/23
 */
  ...
  $.fn.tagedit = function(options) {
    ...
    function inputsToList() {
      ...
      elements.each(function() {
        ...
        if (element_name && element_name.length == 4 && (!options.deleteEmptyItems || $(this).val().length > 0)) {
        ...
        // Set function on the input
          ...
          .each(function() {
                  ...
                  $(this).bind('transformToTag', function(event, id) {
                    ...
                    var checkAutocomplete = !oldValue;
                    // check if the Value ist new
                    var isNewResult = isNew($(this).val(), checkAutocomplete);
                    if (isNewResult[0] === true || (isNewResult[0] === false && typeof isNewResult[1] == 'string')) {

                      if (!oldValue && typeof isNewResult[1] == 'string') {
                        oldValue = true;
                        id = isNewResult[1];
                      }
                                 ...
                                 switch (code) {
                                   case 8: // BACKSPACE
                                     if ($(this).val().length == 0) {
                                       ...
                                       elementToRemove.fadeOut(options.animSpeed,
                                                               function() {
                                                                 elementToRemove.remove();
                                                                 triggerTarget.trigger('tagRemoved');
                                                                 triggerTarget.trigger('tagsChanged');
                                                               });
                                     ...
                                     break;
                                   case 9: // TAB
                                     if ($('ul.ui-autocomplete #ui-active-menuitem').length == 0) {
                                       if ($(this).val().length > 0) {
                                         ...
                                         if (event.shiftKey) {
                                           $(this).trigger("tabToPreviousElement");
                                         } else {
                                           $(this).trigger("tabToNextElement");
                                         }
                                         return false;
                                       }
                                     } else {
          ...
          .click(function(event) {
                   switch (event.target.tagName) {
                     ...
                     case 'SPAN':
                     case 'LI':
                       if (!$(event.target).hasClass('tagedit-listelement-deleted') &&
                           !$(event.target).parent('li').hasClass('tagedit-listelement-deleted')) {
    ...
    function doEdit(event) {
      ...
      element.bind('finishEdit', function(event, doReset) {
        ...
        if (textfield.val().length > 0 && (typeof doReset == 'undefined' || doReset === false) && (isNewResult[0])) {
      ...
      var html = '<input type="text" name="tmpinput" autocomplete="off" value="' + hidden.val() + '" class="tagedit-edit-input" dir="' + options.direction + '"/>';
      ...
      if (options.allowDelete && element.find(':hidden').length > 0 &&
    ...
    function isNew(value, checkAutocomplete) {
      ...
      var compareValue = options.checkNewEntriesCaseSensitive ? value : value.toLowerCase();

      var isNew = true;
      elements.find('li.tagedit-listelement-old input:hidden').each(function() {
        var elementValue = options.checkNewEntriesCaseSensitive ? $(this).val() : $(this).val().toLowerCase();
        if (elementValue == compareValue) {
          isNew = false;
        }
      });

      if (isNew && checkAutocomplete && options.autocompleteOptions.source != false) {
        var result = [];
        ...
        for (var i = 0; i < result.length; i++) {
          var label = options.checkNewEntriesCaseSensitive ? result[i].label : result[i].label.toLowerCase();
