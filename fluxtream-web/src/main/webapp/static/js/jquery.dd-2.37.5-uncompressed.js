...
//
// Chris Bartley <bartley@cmu.edu> made these modifications on 2012.02.07:
// - made it work better with <option> elements which have values but no text
// - changed some CSS
// - changed the arrow image
/*
...
;(function($) {
   ...
   var dd = function(element, options)
   {
	...
	var createA = function(currentOptOption, current, currentopt, tp) {
		...
		a_array[aid] = {html:arrow + sText, value:sValue, text:sValue, index:currentOptOption.index, id:aid};
	...
	var manageSelection = function (obj) {
		...
		if($("#"+childid+ " a."+styles.selected).length==1) { //check if there is any selected
			oldSelectedValue = a_array[$("#"+childid +" a.selected").prop("id")].value;
		};
		if(!ddList) {
	...
	this.close = function() {
				...
				if(wf.opp==true) {
					...
					$("#"+childid).slideUp(50, function(event) {
																fireCloseEvent();
